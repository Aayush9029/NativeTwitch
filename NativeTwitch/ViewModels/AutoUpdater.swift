...
class AutoUpdater: ObservableObject {
    ...
    @AppStorage(StorageStrings.remoteUpdateJson) var remoteUpdateJson = Constants.remoteUpdateURL
...
//                throws unzip error no plans on using that yet.. global catch should suffice
    ...
    func fetchRemoteVersion() async throws -> UpdateModel {
        ...
        let (data, _) = try await URLSession.shared.data(from: URL(string: remoteUpdateJson)!)
